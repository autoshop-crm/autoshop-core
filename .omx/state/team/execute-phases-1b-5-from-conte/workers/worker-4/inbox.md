...
**Team:** execute-phases-1b-5-from-conte
**Role:** executor
**Worker Name:** worker-4

## Your Assigned Tasks

- **Task 4**: verification/read-model coordination.
  Description: verification/read-model coordination.
  Status: pending
  Role: executor
...
1. Load and follow the worker skill from the first existing path:
   ...
   - `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core/.codex/skills/worker/SKILL.md`
   - `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core/skills/worker/SKILL.md` (repo fallback)
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):

   `omx team api send-message --input "{"team_name":"execute-phases-1b-5-from-conte","from_worker":"worker-4","to_worker":"leader-fixed","body":"ACK: worker-4 initialized"}" --json`
...
5. Read the task file for your selected task id at `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core/.omx/state/team/execute-phases-1b-5-from-conte/tasks/task-<id>.json` (example: `task-1.json`)
6. Task id format:
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core/.omx/state/team/execute-phases-1b-5-from-conte/workers/worker-4/status.json`
13. Wait for the next instruction from the lead
14. For legacy team_* MCP tools (hard-deprecated), use `omx team api`; do not pass `workingDirectory` unless the lead explicitly asks (if resolution fails, use leader cwd: `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"execute-phases-1b-5-from-conte","worker":"worker-4"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"execute-phases-1b-5-from-conte","worker":"worker-4","message_id":"<MESSAGE_ID>"}" --json`
...
- from_worker: "worker-4"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: omx team api send-message --input "{"team_name":"execute-phases-1b-5-from-conte","from_worker":"worker-4","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
