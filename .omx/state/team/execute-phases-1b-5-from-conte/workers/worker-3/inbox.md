...
- **Task 3**: approval loop + parts integration,
  Description: approval loop + parts integration,
  Status: pending
  Role: executor
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"execute-phases-1b-5-from-conte","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: worker-3 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/vladislavkovrigin/Projects/IdeaProjects/autoshop-core/.omx/state/team/execute-phases-1b-5-from-conte/workers/worker-3/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"execute-phases-1b-5-from-conte","worker":"worker-3"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"execute-phases-1b-5-from-conte","worker":"worker-3","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"execute-phases-1b-5-from-conte","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
